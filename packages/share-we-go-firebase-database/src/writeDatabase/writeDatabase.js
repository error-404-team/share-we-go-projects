...
export function writeLocationPrivateData(userId, position, name_address, place_id) {
    firebase.database().ref(`location/${userId}/`).set({
        location_id: userId,
        lat: position.lat,
        lng: position.lng,
...
export function writeLocationNearbyUsersData(userId,group_share_id,displayName,photoURL, email, position, name_address) {
    firebase.database().ref(`location_near_by_users/${userId}/${group_share_id}`).set({
...
export function writeSearchLocationNearbyUsersData(userId,group_share_id,displayName,photoURL, email, position, name_address) {
    firebase.database().ref(`search_location_near_by_users/${userId}/${group_share_id}`).set({
        group_share_id: group_share_id,
        lat: position.lat(),
        lng: position.lng(),
        name_address: name_address,
...
export function writeShareMyWayNearbyUsersData(userId,group_share_id,displayName,photoURL, email, start_position,end_position, start_address, end_address) {
    firebase.database().ref(`share_my_way_near_by_users/${userId}/${group_share_id}`).set({
        group_share_id: group_share_id,
...
export function writeDestinationUsersData(userId,position, end_address) {
    firebase.database().ref(`destination_users/${userId}/`).set({
        location_id: userId,
        lat: position.lat(),
        lng: position.lng(),
        end_address: end_address
...
export function writeCreateGroupShareUserData(userId, start_lat, start_lng, end_lat, end_lng,start_address, end_address, start_time, end_time, user_num) {
    firebase.database().ref(`group_share_user/${userId}`).set({
...
export function addUserToGroupShareData(userId, user) {
    firebase.database().ref(`group_shareuserId/group_share`).push(user);
}