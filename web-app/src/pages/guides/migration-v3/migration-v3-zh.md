...
> 此文档尚未完成。 您是否已经升级了站点并且遇到了一些并没有在此涉及的问题？ [请在 GitHub 添加您的更改](https://github.com/mui-org/material-ui/blob/master/web-app/src/pages/guides/migration-v3/migration-v3.md)。
...
对于 React 版本的最低要求是从 `react@^16.3.0` 升级到 `react@^16.8.0`。 这样一来我们能够依赖 [Hooks](https://reactjs.org/web-app/hooks-intro.html) 的功能（我们已经不再使用 class API）。
...
- 请移除 `withTheme()` 中的第一个可选的参数。 第一个参数本是作为未来的可能的选项的一个占位符。 我们从未发现有需要它的情况。 是时候来删除这个参数了。 它与[emotion 的 API](https://emotion.sh/web-app/introduction) 以及 [styled-components 的 API ](https://www.styled-components.com) 相匹配。
